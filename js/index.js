...
In summary, the `getItems` function fetches JSON data from the "db.json" file, located in the "data_base" subfolder. After the data is successfully retrieved, it conditionally calls `renderCategory` and `renderItem` functions based on the provided `category` and `item_` arguments, respectively.
...
3. The `fetch` function returns a Promise that resolves to the Response object. We can chain methods to this Promise using the `.then()` function.
...
6. If `category` is truthy (i.e., it has a value), the `renderCategory` function is called with the `data` object as an argument. This function is not provided in the code snippet, so its implementation is unknown.

7. If `item_` is truthy (i.e., it has a value), the `renderItem` function is called with the `data` object as an argument. Similar to the `renderCategory` function, the `renderItem` function is not provided in the code snippet.
...
/* 
`renderCategory` displays the categories page. is executed when the items are retrieved and it filters them based on the category value. Then, it dynamically adds the filtered items using the DOM (essentially using `createElement` and `appendChild` to create HTML elements without directly writing them in the HTML). This way, with a single function, it can generate all the categories instead of creating them individually. 
*/
...
function renderItem(data) {
  ...
  const render_js_content =
    document.querySelector(
      "#render_js_content"
    ); /* In the HTML, there is a `<div>` with the ID "#render_js_content" where the categories and individual items will be dynamically added. */
  const article = document.createElement("article");
...
Within this function, references are made to other 
functions that edit the quantity of an item or remove it.
*/
